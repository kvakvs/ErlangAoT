...
enum class DirectiveKind : std::uint8_t {
    define, undef, include, include_lib, ifdef, ifndef, if_condition, elif,
    else_branch, endif, feature, error, warning
};
struct Definition {
...
struct MacroName {
    // Retain the operand location for later definedness/undefinition diagnostics.
    Token name;
};
struct TokenOperand {
...
using DirectiveOperand = std::variant<std::monostate, Definition, MacroName,
    TokenOperand, FeatureSetting>;
struct Directive {
...
// Recognize only supported names following an attribute dash, including keywords.
std::optional<DirectiveKind> directive_kind(std::span<const Token> tokens);
// Parse a complete recognized form transactionally; malformed syntax is a value.
