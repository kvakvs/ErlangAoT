...
std::optional<Encoding> encoding_comment(std::string_view line) {
    ...
    if (!std::regex_search(line.begin(), line.end(), match, marker)) {
        return std::nullopt;
    }
...
Encoding detect_encoding(std::string_view bytes) {
    for (int line = 0; line < 2; ++line) {
        ...
        if (const auto encoding = encoding_comment(current)) {
            return *encoding;
        }
...
char32_t decode(std::string_view bytes, std::size_t &offset) {
    ...
    for (unsigned part = 1; part < length; ++part) {
        ...
        if ((byte & 0xc0) != 0x80) {
            throw EncodingError(start, "invalid UTF-8 continuation");
        }
        value = (value << 6) | (byte & 0x3f);
    }
    if (!scalar(value, length)) {
        throw EncodingError(start, "invalid Unicode scalar");
    }
    offset += length;
    return value;
}
...
SourcePtr SourceManager::read(const std::filesystem::path &path) {
    ...
    if (input.bad()) {
        throw std::runtime_error("failed reading source: " + path.string());
    }
