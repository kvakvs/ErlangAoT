...
bool variable_start(char32_t value)
{
    return value == U'_' || (value >= U'A' && value <= U'Z')
        || (value >= U'À' && value <= U'Þ' && value != U'×');
}
bool whitespace(char32_t value)
{
    return value <= 32 || (value >= 128 && value <= 160);
}
std::size_t word_length(std::u32string_view input)
{
    const auto latin = (bp::char_(U'À', U'ÿ') - bp::char_(U"×÷"));
    const auto part = bp::char_(U'a', U'z') | bp::char_(U'A', U'Z')
        | bp::char_(U'0', U'9') | bp::char_(U"_@") | latin;
...
Lexer::Lexer(SourcePtr source, bool comments)
    : source_(std::move(source)), comments_(comments),
      keywords_{U"after", U"begin", U"case", U"try", U"cond", U"catch", U"andalso",
        U"orelse", U"end", U"fun", U"if", U"let", U"of", U"receive", U"when",
        U"bnot", U"not", U"div", U"rem", U"band", U"and", U"bor", U"bxor",
        U"bsl", U"bsr", U"or", U"xor", U"maybe", U"else"}, logical_file_(source_->name) {}
...
Token Lexer::token(TokenKind kind, TokenValue value, std::size_t begin, std::size_t end) const
{
    ...
    return {kind, std::move(value), {source_, begin, end},
...
{
    while (!rest().empty()) {
        if (whitespace(rest().front())) { ++cursor_; previous_string_ = false; continue; }
        if (rest().front() != U'%') { break; }
...
Token Lexer::word()
{
    ...
    if (kind == TokenKind::atom && keywords_.contains(text)) { kind = TokenKind::keyword; }
    if (text.size() > 255) { fail(DiagnosticCode::invalid_character, "name exceeds 255 characters", begin); }
...
{
    if (!pending_.empty()) {
        auto result = std::move(pending_.front()); pending_.pop_front(); return result;
    }
    if (auto comment = trivia()) { return comment; }
    if (rest().empty()) { return std::nullopt; }
    const auto first = rest().front();
    if (atom_start(first) || variable_start(first)) { previous_string_ = false; return word(); }
    if (first >= U'0' && first <= U'9') { previous_string_ = false; return number(); }
    return literal();
}
