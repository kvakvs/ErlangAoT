...
bool valid_scalar(char32_t value)
{
...
{
    if (value >= U'0' && value <= U'9') { return static_cast<unsigned>(value - U'0'); }
    if (value >= U'a' && value <= U'f') { return static_cast<unsigned>(value - U'a') + 10; }
    if (value >= U'A' && value <= U'F') { return static_cast<unsigned>(value - U'A') + 10; }
    return 16;
}
struct EscapeDigits {
...
char32_t numeric_escape(std::u32string_view& input, EscapeDigits syntax)
{
    ...
    while (!input.empty() && count < syntax.limit) {
        ...
        if (digit >= syntax.base) { break; }
        if (value > 0x10ffff / syntax.base) { throw std::invalid_argument("escape exceeds Unicode range"); }
        value = value * syntax.base + digit;
        input.remove_prefix(1); ++count;
    }
    if (count == 0 || !valid_scalar(value)) { throw std::invalid_argument("invalid numeric escape"); }
    return value;
}
...
char32_t hexadecimal(std::u32string_view& input)
{
    if (input.starts_with(U"{")) {
        ...
        if (!input.starts_with(U"}")) { throw std::invalid_argument("missing hexadecimal closing brace"); }
        input.remove_prefix(1); return value;
    }
    const auto before = input.size();
    const auto value = numeric_escape(input, {16, 2});
    if (before - input.size() != 2) { throw std::invalid_argument("hexadecimal escape needs two digits"); }
    return value;
}
// Translate a control-character escape using Erlang's accepted alphabet.
char32_t caret(std::u32string_view& input)
{
    if (input.empty()) { throw std::invalid_argument("incomplete control escape"); }
    const auto value = input.front(); input.remove_prefix(1);
    if (value == U'?') { return 127; }
    if ((value >= U'@' && value <= U'_') || (value >= U'a' && value <= U'z')) { return value & 31; }
    throw std::invalid_argument("invalid control escape");
}
// Decode one escape after the backslash; unknown simple escapes quote themselves.
char32_t escaped(std::u32string_view& input)
{
    if (input.empty()) { throw std::invalid_argument("incomplete escape"); }
    if (input.front() >= U'0' && input.front() <= U'7') { return numeric_escape(input, {8, 3}); }
    const auto value = input.front(); input.remove_prefix(1);
    if (value == U'x') { return hexadecimal(input); }
    if (value == U'^') { return caret(input); }
...
{
    const auto begin = cursor_++;
    if (rest().empty()) { fail(DiagnosticCode::unterminated_literal, "missing character after $", begin); }
...
Token Lexer::quoted(char32_t delimiter, bool verbatim, TokenKind kind)
{
    ...
    while (!rest().empty() && rest().front() != delimiter) {
        if (!verbatim && rest().front() == U'\\') { value.push_back(escape()); }
        else { value.push_back(source_->text[cursor_++]); }
    }
    if (rest().empty()) { fail(DiagnosticCode::unterminated_literal, "unterminated quoted literal", begin); }
    ++cursor_;
    if (kind == TokenKind::atom && value.size() > 255) { fail(DiagnosticCode::invalid_character, "atom exceeds 255 characters", begin); }
...
std::u32string Lexer::strip_indent(std::u32string_view text, Indentation indent, std::size_t begin) const
{
    ...
    while (!remaining.empty()) {
        ...
        if (line.starts_with(indent.prefix)) { line.remove_prefix(indent.prefix.size()); }
        else if (line != U"\n" && line != U"\r\n") {
    ...
    if (result.ends_with(U"\r")) { result.pop_back(); }
    return result;
}
...
{
    const auto begin = cursor_;
    while (rest().starts_with(U"\"")) { ++cursor_; }
    const std::u32string delimiter(cursor_ - begin, U'"');
    const auto first_line = rest().find(U'\n');
    if (first_line == std::u32string_view::npos) { cursor_ = source_->text.size(); fail(DiagnosticCode::unterminated_literal, "unterminated multiline literal", begin); }
...
Token Lexer::triple_content(std::size_t begin, std::u32string_view delimiter, bool verbatim)
{
    ...
    while (!rest().empty() && !rest().starts_with(delimiter)) {
        if (!verbatim && rest().front() == U'\\') { static_cast<void>(escape()); }
        else { ++cursor_; }
    }
    if (rest().empty()) { fail(DiagnosticCode::unterminated_literal, "unterminated multiline literal", begin); }
    ...
    if (!std::ranges::all_of(indent, whitespace)) { fail(DiagnosticCode::string_indentation, "invalid closing indentation", cursor_); }
    auto value = strip_indent(prefix.substr(0, indent_begin), Indentation{indent}, begin);
    cursor_ += delimiter.size();
    if (!verbatim) { value = unescape(value); }
...
Token Lexer::sigil()
{
    ...
    if (rest().empty() || closing(rest().front()) == 0) { fail(DiagnosticCode::invalid_character, "invalid sigil delimiter", begin); }
    const bool escaped_sigil = name == U"s" || name == U"b";
    auto value = rest().starts_with(U"\"\"\"") ? triple(!escaped_sigil)
...
Token Lexer::literal()
{
    ...
    if (first == U'"') {
        if (previous_string_) { fail(DiagnosticCode::adjacent_strings, "adjacent strings need whitespace", cursor_); }
        previous_string_ = true;
        return rest().starts_with(U"\"\"\"") ? triple(true) : quoted(U'"', false, TokenKind::string);
    }
    previous_string_ = false;
    if (first == U'\'') { return quoted(U'\'', false, TokenKind::atom); }
    if (first == U'$') { return character(); }
    if (first == U'~') { return sigil(); }
    return punctuation();
}
} // namespace erlang_aot
