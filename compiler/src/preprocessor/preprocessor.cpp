...
std::optional<Span> misplaced(std::span<const Token> tokens) {
    if (tokens.front().text() == U"-") {
        return std::nullopt;
    }
    const auto arrow = std::ranges::find_if(
        tokens, [](const Token &token) { return token.kind == TokenKind::symbol && token.text() == U"->"; });
    if (arrow == tokens.end()) {
        return std::nullopt;
    }
    ...
    if (!directive_kind(tokens)) {
        if (const auto location = misplaced(tokens)) {
            return error({DiagnosticCode::misplaced_directive,
                          "preprocessing directive must start a separate form",
                          *location,
...
std::optional<PreprocessorEvent> DirectiveReader::next() {
    ...
    if (tokens.back().kind != TokenKind::dot) {
        ...
        return error({DiagnosticCode::missing_terminator,
                      "expected form-ending '.'",
                      {frame.source, end, end},
                      {tokens.front().spelling}});
    }
