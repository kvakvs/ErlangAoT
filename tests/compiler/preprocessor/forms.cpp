...
// Read the first directive while keeping all token source ownership in its value.
...
// Multiline envelopes retain incomplete bodies, empty bodies, and exact spellings.
void definitions()
{
    ...
    require(body.name.spelling.source->spelling(body.name.spelling.begin, body.name.spelling.end) == "'NAME'", "owned original spelling");
...
// Exercise every supported directive family without performing its later effects.
void directives()
{
    const std::pair<const char*, DirectiveKind> cases[]{
        {"-undef(X).", DirectiveKind::undef}, {"-ifdef(x).", DirectiveKind::ifdef},
        {"-ifndef('x').", DirectiveKind::ifndef}, {"-if(defined(X) andalso ?COND).", DirectiveKind::if_condition},
        {"-elif(length([]) =:= 0).", DirectiveKind::elif}, {"-else.", DirectiveKind::else_branch},
        {"-endif.", DirectiveKind::endif}, {"-'else'.", DirectiveKind::else_branch},
        ...
        {"-error({reason, ?DETAIL}).", DirectiveKind::error}, {"-warning(\"text\").", DirectiveKind::warning}};
    for (const auto& [text, kind] : cases) { require(directive(text).kind == kind, "directive kind"); }
...
// Non-preprocessing attributes and ordinary forms keep lexical categories and order.
void ordinary_forms()
{
    ...
    for (const auto& event : result) { require(std::holds_alternative<OrdinaryForm>(event), "ordinary attribute passthrough"); }
    require(std::holds_alternative<OrdinaryForm>(events("-custom(fun() ->\n -define(X,1) end).").front()), "unknown attribute remains opaque");
    SourceManager sources;
...
{
    const char* cases[]{"-define.", "-define(1, ok).", "-define(X ok).", "-define(X(a), ok).",
        "-define(X(A,), ok).", "-define(X, ok.", "-undef().", "-undef(A,B).", "-ifdef(42).",
        "-ifndef(A) extra.", "-else().", "-endif(no).", "-if().", "-elif().",
        "-include(42).", "-include(?).", "-include_lib(foo).", "-feature(foo, nope).",
        "-feature(FOO, enable).", "-error().", "-warning().", "-define'('X,ok)."};
...
// Missing dots, misplaced envelopes and scanner failures retain stable categories.
void diagnostics()
{
    ...
    require(std::get<Diagnostic>(events("f() -> ok").front()).code == DiagnosticCode::missing_terminator, "missing ordinary dot");
...
// Failure status and cursors are independent even when sessions are interleaved.
void isolation()
{
    SourceManager sources;
...
// Truncated and corrupted forms terminate even when literals contain apparent dots.
void bounded_recovery()
{
    ...
    for (const auto* text : malformed) { events(std::string(text) + ".\nnext()."); }
}
int main()
{
    definitions(); directives(); ordinary_forms(); malformed_envelopes();
    diagnostics(); isolation(); bounded_recovery();
}
