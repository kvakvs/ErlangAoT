...
void require(bool value) {
    if (!value) {
        throw std::runtime_error("lexer assertion failed");
    }
}
...
void source_locations() {
    ...
    try {
        manager.add("invalid.erl", "\xc0\x80");
        require(false);
...
// Check lexical errors with stable categories, valid spans, and finite
// progress.
void invalid_literals() {
    using enum erlang_aot::DiagnosticCode;
    const std::pair<std::string, erlang_aot::DiagnosticCode> cases[]{{"\"unterminated", unterminated_literal},
                                                                     {"$", unterminated_literal},
                                                                     {"$\\x{}", invalid_escape},
                                                                     {"$\\x{110000}", invalid_escape},
                                                                     {"1#0", invalid_number},
                                                                     {"16#", invalid_number},
                                                                     {"1.0e+", invalid_number},
                                                                     {"1_", invalid_number},
                                                                     {"1a", invalid_number},
                                                                     {"1.0foo", invalid_number},
                                                                     {"16#1.x", invalid_number},
                                                                     {"16#1_.0", invalid_number},
                                                                     {"\"a\"\"b\"", adjacent_strings},
                                                                     {"λ", invalid_character}};
